...
def involved(courses, person):
    course_involvement = {}
    for semester, course_dict in courses.items():
        for course, details in course_dict.items():
            if person in details.values():
                if (semester in course_involvement.keys()):
                    course_involvement[semester].append(course)
                else:
                    course_involvement.update({ semester: [course] })
    return course_involvement
# For example:
